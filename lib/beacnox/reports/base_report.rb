  ...
  module Reports
    class BaseReport
      ...
      def calculate_data
        ...
        offset = 0 # Beacnox::Reports::BaseReport.time_in_app_time_zone(now).utc_offset
        current = stop - Beacnox.duration
        ...
        # add blank columns
        while current <= stop
          key = current.strftime(Beacnox::FORMAT)
      ...
      #   ....
      # }
      def nil_data(duration = Beacnox.duration)
        @nil_data ||= begin
          result = {}
          now = Beacnox::Utils.time
          now = now.change(sec: 0, usec: 0)
          ...
          while current <= stop
            current.strftime(Beacnox::FORMAT)
      ...
      #   1732125550000 => 0,
      # }
      def nullify_data(input, duration = Beacnox.duration)
