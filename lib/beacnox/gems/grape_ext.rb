    ...
    class GrapeExt
      def self.init
        ActiveSupport::Notifications.subscribe(/grape/) do |name, start, finish, id, payload|
          ...
          CurrentRequest.current.record ||= Beacnox::Models::GrapeRecord.new(request_id: CurrentRequest.current.request_id)
          CurrentRequest.current.record.datetimei ||= now.to_i
          CurrentRequest.current.record.datetime ||= now.strftime(Beacnox::FORMAT)
