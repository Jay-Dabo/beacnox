  ...
  module Models
    class RequestRecord < BaseRecord
      ...
      def self.find_by(request_id:)
        keys, values = Beacnox::Utils.fetch_from_redis("performance|*|request_id|#{request_id}|*")

        return nil if keys.blank?
        return nil if values.blank?

        Beacnox::Models::RequestRecord.from_db(keys[0], values[0])
      end
      ...
      # END|1.0.0
      # = {"view_runtime":null,"db_runtime":0,"duration":27.329741000000002,"http_referer":null,"custom_data":null,"exception":"ZeroDivisionError divided by 0","backtrace":["/root/projects/beacnox/test/dummy/app/controllers/account/site_controller.rb:17:in `/'","/root/projects/beacnox/test/dummy/app/controllers/account/site_controller.rb:17:in `crash'","/usr/local/rvm/gems/ruby-2.6.3/gems/actionpack-6.1.3.1/lib/action_controller/metal/basic_implicit_render.rb:6:in `send_action'"]}
      # value = JSON
      ...
      def record_hash
        {
          ...
          path: path,
          request_id: request_id,
          datetime: Beacnox::Utils.from_datetimei(datetimei.to_i),
          datetimei: datetimei,
      ...
      def save
        key = "performance|controller|#{controller}|action|#{action}|format|#{format}|status|#{status}|datetime|#{datetime}|datetimei|#{datetimei}|method|#{method}|path|#{path}|request_id|#{request_id}|END|#{Beacnox::SCHEMA}"
        value = {
          view_runtime: view_runtime,
          db_runtime: db_runtime,
          duration: duration,
